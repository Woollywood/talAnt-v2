...
class xajax
{
	 ...
	 */
	function xajax_s($sRequestURI="",$sWrapperPrefix="xajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false)
	{
	...
	{
		$sVar = 'b' . ucfirst($name);
		if (array_key_exists($sVar, (array)get_object_vars($this))) {
	...
	{	
		...
		//array("123"=>'312')
//file_put_contents('mass_test.txt',."\n", FILE_APPEND);
		
		if (array_key_exists($sFunctionName,(array)$this->aFunctionIncludeFiles))
		{
			ob_start();
		...
		if ($bFoundFunction) {
			...
			if (!array_key_exists($sFunctionName, (array)$this->aFunctions))
			{
		...
		if ($this->bErrorHandler && !empty( $GLOBALS['xajaxErrorHandlerText'] )) {
			...
			$oErrorResponse->xajaxResponse_s();
	...
	function _isObjectCallback($sFunction)
	{
		//array("123"=>'312')
		//
		if (array_key_exists($sFunction, (array)$this->aObjects)) return true;
		return false;
	}
